...
##Only for use on MELT Split calls, which have been modified to include the assumed mLucifugus genotype, and module
...
def read_melt(FILENAME):
	...
	for line in open(FILENAME):
		...
		try:
			my_dict[line[0]].append((START,STOP,ASSESS,TYPE,LENGTH,ORIENTATION,GTYPE1,GTYPE2,GTYPE3,GTYPE4,GTYPE5,GTYPE6,GTYPE7,GTYPE8,GTYPE9,GTYPE10,GTYPE11,GTYPE12,MODULE))
		except KeyError:
			my_dict[CHROM] = [(START,STOP,ASSESS,TYPE,LENGTH,ORIENTATION,GTYPE1,GTYPE2,GTYPE3,GTYPE4,GTYPE5,GTYPE6,GTYPE7,GTYPE8,GTYPE9,GTYPE10,GTYPE11,GTYPE12,MODULE)]
	return my_dict
			...
			if len(INSERTION_CLUSTER) > 1:
				...
				# if orientation is same in all:
					if len(df[5].unique()) == 1:
						...
						if all(len(df[column].unique()) == 1 for column in df.columns[6:17]):
							...
							if len(df[0].unique())== 1 or all(abs(x[0]-z[0]) <= 15 for x,z in itertools.combinations(INSERTION_CLUSTER,r=2)):
								...
								elif all(["Ves" in x for x in df[3]]):
