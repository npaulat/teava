...
def get_args():
	...
	parser.add_argument('-l', '--telist', type=str, help='File containing list of TEs to analyze with MELT', required=True)
	...
	return TE_LIST, REF, RATE, PROC, OUTDIR, QUEUE
	
TE_LIST, REF, RATE, PROC, OUTDIR, QUEUE = get_args()
...
if QUEUE == 'hrothgar':
	with open(QSUB1FILENAME, 'w') as f:
		...
		f.write('\n')
		f.write('MELT_HOME=/lustre/scratch/npaulat/MELTv2.1.5\n')
		f.write('WORKDIR=' + MUT_RATE_DIR + '/del\n')
		f.write('FILEDIR=/lustre/scratch/npaulat/MELTv2.1.5/references\n')
		f.write('\n')
		...
		f.write('for i in Aust Bran80 Cili Davi Occu Sept_TTU Sept_USDA Thys Veli Vive Yuma; do java -Xmx2G -jar $MELT_HOME/MELT.jar Deletion-Genotype -w $WORKDIR -bamfile $FILEDIR/bams/${i}_paired.sorted.bam -bed $FILEDIR/beds/allTEs_filtered.bed -h $FILEDIR/myoLuc2.fa; done\n')
		f.write('\n')
		...
		f.write('java -Xmx2G -jar $MELT_HOME/MELT.jar Deletion-Merge -bed $FILEDIR/beds/allTEs_filtered.bed -mergelist $WORKDIR/del_list.txt -h $FILEDIR/myoLuc2.fa -o $WORKDIR\n')
		f.write('\n')
...
with open("/lustre/scratch/npaulat/MELTv2.1.5/references/te_fasta_names.txt", "r") as d:
	FASTAS = d.read().split(" ")

with open("/lustre/scratch/npaulat/MELTv2.1.5/references/zip_te_names.txt", "r") as d:
	ZIPS = d.read().split(" ")
...
for TE, FASTA, ZIP in zip(TES, FASTAS, ZIPS):
	...
	if QUEUE == 'hrothgar':
		with open(QSUB2FILENAME, 'w') as h:
			...
			h.write('\n')
			h.write('MELT_HOME=/lustre/scratch/npaulat/MELTv2.1.5\n')
			h.write('WORKDIR=' + MUT_RATE_DIR + '\n')
			h.write('FILEDIR=/lustre/scratch/npaulat/MELTv2.1.5/references\n')
			h.write('\n')
			...
			h.write('java -Xmx1G -jar $MELT_HOME/MELT.jar BuildTransposonZIP $FILEDIR/fastas/' + FASTA + '.fa' + ' $FILEDIR/beds/' + TE + '.out.bed $ZIP_NAME ' + str(RATE) + '\n')
			h.write('\n')
			h.write('cd $WORKDIR\n')
			h.write('\n')
			h.write('#=== ' + TE + ' discovery\n')
			h.write('echo "Begin IndivAnalysis."\n')
			h.write('for i in Aust Bran80 Cili Davi Occu Sept_TTU Sept_USDA Thys Veli Vive Yuma; do java -Xmx6G -jar $MELT_HOME/MELT.jar IndivAnalysis -w $WORKDIR/' + ZIP + ' -bamfile $FILEDIR/bams/${i}_paired.sorted.bam -c 14 -h $FILEDIR/myoLuc2.fa -t $FILEDIR/zips/$ZIP_FILE -r 150; done\n')
			h.write('\n')
			h.write('echo "Begin GroupAnalysis."\n')
			h.write('java -Xmx6G -jar $MELT_HOME/MELT.jar GroupAnalysis -discoverydir $WORKDIR/' + ZIP + ' -h $FILEDIR/myoLuc2.fa -n $FILEDIR/mLuc_genetrack.bed -t $FILEDIR/zips/$ZIP_FILE -w $WORKDIR/' + ZIP + ' -r 150\n')
			h.write('\n')
			h.write('echo "Begin Genotyping."\n')
			h.write('for i in Aust Bran80 Cili Davi Occu Sept_TTU Sept_USDA Thys Veli Vive Yuma; do java -Xmx6G -jar $MELT_HOME/MELT.jar Genotype -w $WORKDIR/' + ZIP + ' -bamfile $FILEDIR/bams/${i}_paired.sorted.bam -h $FILEDIR/myoLuc2.fa -t $FILEDIR/zips/$ZIP_FILE -p $WORKDIR/' + ZIP + '; done\n')
			h.write('\n')
			...
			h.write('java -Xmx6G -jar $MELT_HOME/MELT.jar MakeVCF -genotypingdir $WORKDIR/' + ZIP + ' -h $FILEDIR/myoLuc2.fa -t $FILEDIR/zips/$ZIP_FILE -w $WORKDIR/' + ZIP + ' -p $WORKDIR/' + ZIP + '\n')
			h.write('\n')
